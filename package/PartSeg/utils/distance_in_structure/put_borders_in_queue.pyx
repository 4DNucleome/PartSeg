...
cdef void put_borders_in_queue_indexed(np.ndarray[object_area_types, ndim=3] object_area, my_queue[Point] & current_points,
                                      np.ndarray[component_types, ndim=3] base_object,
                                      np.ndarray[int8_t, ndim=2] neighbourhood, int index):
    cdef Size x_size, y_size, z_size, x, y, z, xx, yy, zz
    cdef Point p, p1
    cdef char neigh_length = neighbourhood.shape[0]
    z_size = object_area.shape[0]
