...
extensions = [
    Extension('PartSegCore.sprawl_utils.euclidean_cython',
    ...
    Extension('PartSegCore.sprawl_utils.path_sprawl_cython',
              sources=["package/PartSegCore/sprawl_utils/path_sprawl_cython.pyx"],
              include_dirs=[np.get_include()] + [os.path.join(package_dir, "PartSegCore", "sprawl_utils")],
              language='c++', extra_compile_args=["-std=c++11"], extra_link_args=["-std=c++11"]),
    Extension('PartSegCore.sprawl_utils.sprawl_utils',
              sources=["package/PartSegCore/sprawl_utils/sprawl_utils.pyx"],
              include_dirs=[np.get_include()] + [os.path.join(package_dir, "PartSegCore", "sprawl_utils")],
              language='c++', extra_compile_args=["-std=c++11"], extra_link_args=["-std=c++11"]),
    Extension('PartSegCore.sprawl_utils.fuzzy_distance',
...
setuptools.setup(
    ...
    install_requires=['numpy>=1.16.0', "tifffile>=2019.7.26", "czifile>=2019.4.20", "oiffile>=2019.1.1",
                      ...
                      'sentry_sdk==0.13.4', qt_string, 'six>=1.11.0', 'h5py>=2.7.1', 'packaging>=17.1',
